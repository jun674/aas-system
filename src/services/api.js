...
apiClient.interceptors.request.use(
  (config) => {
    // 세션 기반 인증에서는 Bearer 토큰을 사용하지 않음
...
export const dataAPI = {
  ...
  getAllAAS: async (page = 1, keyword = null) => {
    try {
      ...
      if (response.data.message && Array.isArray(response.data.message)) {
        response.data.message = response.data.message.filter((aas) => aas.idShort !== 'Component')
  ...
  // 키워드별 AAS 조회 (메뉴 전용)
  getAASByKeyword: async (keyword, page = 1) => {
    // page 파라미터 추가
    try {
  ...
  getAllConceptDescriptions: async (page = 1) => {
    try {
      ...
      throw new Error(
        `전체 ConceptDescription 조회 실패: ${error.response?.status} ${error.message}`,
      )
    }
  },
...
export const searchAPI = {
  ...
  searchByFilter: async (filterType, value) => {
    ...
    try {
      ...
      const response = await apiClient.get(
        `/repository/search/${endpoint}?value=${encodeURIComponent(value)}`,
      )
      return response.data
    } catch (error) {
  ...
  searchByKeyword: async (entityType, keyword, page = 1) => {
    ...
    try {
      ...
      throw new Error(
        `Keyword search on ${endpoint} failed: ${error.response?.status} ${error.message}`,
      )
    }
  },
