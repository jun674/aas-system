    ...
    >
      ...
      <div class="quick-search-buttons">
        <button
          ...
          :disabled="loading"
          :class="{ 'loading': loading && activeQuickSearch === 'inputpowervoltage-380' }"
        >
          <i v-if="loading && activeQuickSearch === 'inputpowervoltage-380'" class="fas fa-spinner fa-spin me-1"></i>
        ...
        <button
          ...
          class="btn btn-outline-info btn-sm"
          :disabled="loading"
          :class="{ 'loading': loading && activeQuickSearch === 'numberofphases-Three' }"
        >
          <i v-if="loading && activeQuickSearch === 'numberofphases-Three'" class="fas fa-spinner fa-spin me-1"></i>
        ...
        <button
          ...
          class="btn btn-outline-info btn-sm"
          :disabled="loading"
          :class="{ 'loading': loading && activeQuickSearch === 'dutycycle-60' }"
        >
          <i v-if="loading && activeQuickSearch === 'dutycycle-60'" class="fas fa-spinner fa-spin me-1"></i>
        ...
        <button
          ...
          class="btn btn-outline-info btn-sm"
          :disabled="loading"
          :class="{ 'loading': loading && activeQuickSearch === 'inputcapacity/kw-6.5' }"
        >
          <i v-if="loading && activeQuickSearch === 'inputcapacity/kw-6.5'" class="fas fa-spinner fa-spin me-1"></i>
...
export default {
  ...
  emits: ['search', 'reset', 'filter-type-change', 'update:filterType', 'update:filterValue'],

  setup(props, { emit }) {
    const isMobile = ref(false)
    const activeQuickSearch = ref(null)
    ...
    const quickSearch = (filterType, value) => {
      activeQuickSearch.value = `${filterType}-${value}`
      emit('update:filterType', filterType)
      emit('update:filterValue', value)
      // 약간의 지연 후 검색 실행 (상태 업데이트 보장)
      setTimeout(() => {
        emit('search')
      }, 10)
      // 검색 완료 후 activeQuickSearch 초기화
      setTimeout(() => {
        activeQuickSearch.value = null
      }, 3000)
    }

    const handleFilterTypeChange = (event) => {
      emit('update:filterType', event.target.value)
      emit('filter-type-change')
    }

    const handleFilterValueChange = (event) => {
      emit('update:filterValue', event.target.value)
    }
