    ...
    <div class="signup-container">
      ...
      <form @submit.prevent="handleSignup" class="signup-form">
        <div class="form-group">
          ...
          <span v-if="signupError" class="error-message">
            <i class="fas fa-times-circle"></i> {{ signupError }}
          </span>
        </div>
...
const signupError = ref('') // 회원가입 에러 메시지
...
const checkDuplicate = async () => {
  ...
  signupError.value = ''
  try {
    const response = await authService.checkUsername(formData.userId)
    duplicateExists.value = response.isTaken
  } catch (error) {
    console.error('Failed to check duplicate ID:', error)
    signupError.value = error.message || 'Failed to check ID.'
    duplicateExists.value = true // 실패 시 안전하게 중복으로 처리
  } finally {
...
const handleSignup = async () => {
  if (!isFormValid.value) {
    if (!duplicateChecked.value) {
      signupError.value = 'Please check your User ID availability.'
    }
    return
  }
  ...
  try {
    ...
    alert('Sign up completed successfully! Please log in.')
    router.push('/')
  } catch (error) {
